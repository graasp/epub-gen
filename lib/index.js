...
(function() {
  ...
  EPub = class EPub {
    constructor(options, output) {
      ...
      this.options.images = [];
      this.options.content = _.map(this.options.content, function(section, sectionIndex) {
        section.id = `item_${sectionIndex}`;

        section.coverContent = self.handlePage(section.coverContent, sectionIndex, 0);
        section.content = _.map(section.content, function(content, index) {return self.handlePage(content, sectionIndex, index)});

        return section;
      });

      if (this.options.backCover) {
        // set back cover title used for file path
        this.options.backCover.title = "Back Cover";
        this.options.backCover = self.handlePage(options.backCover, 0, 0, false);
        this.options.backCover.data = '<div id="backcover">' + this.options.backCover.data + '</div>'; 
      }
    ...
    handlePage(content, sectionIndex, index, displayTitle = true) {
        var self = this;
        var $, allowedAttributes, allowedXhtml11Tags, titleSlug;
        if (!content.filename) {
          titleSlug = uslug(removeDiacritics(content.title || "no title"));
          content.href = `${sectionIndex}_${index}_${titleSlug}.xhtml`;
          content.filePath = path.resolve(self.uuid, `./OEBPS/${sectionIndex}_${index}_${titleSlug}.xhtml`);
        } else {
        ...
        content.id = `item_${sectionIndex}_${index}`;
        ...
        content.displayTitle = displayTitle;
        //fix Author Array
    ...
    createPageFile(content) {
      var data;
      var self = this;
      data = `${self.options.docHeader}\n  <head>\n  <meta charset="UTF-8" />\n  <title>${entities.encodeXML(content.title || '')}</title>\n  <link rel="stylesheet" type="text/css" href="style.css" />\n  </head>\n<body>`;
      data += content.title && self.options.appendChapterTitles && content.displayTitle ? `<h1>${entities.encodeXML(content.title)}</h1>` : "";
    ...
    generateTempFile() {
      ...
      _.each(this.options.content, function(section) {
        self.createPageFile(section.coverContent);
        _.each(section.content, function(content) {
            self.createPageFile(content);
          });
      });

      
      if (this.options.backCover) {
        self.createPageFile(this.options.backCover);
      }
